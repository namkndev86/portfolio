...
export function SectionHeader({ badge, title, subtitle, align = 'center' }: SectionHeaderProps) {
  ...
  return (
    <FadeIn className={`flex flex-col mb-12 ${alignment}`}>
      {badge && (
        <span className="inline-block rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary border border-primary/20 mb-3 tracking-wide uppercase">
          {badge}
        </span>
      )}
