  ...
  return (
    <div className="py-16 sm:py-24 bg-slate-950 min-h-screen">
      <Container>
        ...
        <div className="relative max-w-4xl mx-auto mt-16 pl-6 sm:pl-8 border-l-2 border-slate-800 space-y-12">
          {MOCK_EXPERIENCES.map((exp, idx) => (
            <FadeIn key={exp.id} delay={idx * 0.1} className="relative group">
              {/* Timeline dot marker */}
              <div className="absolute -left-[31px] sm:-left-[39px] top-1.5 h-5 w-5 rounded-full border-4 border-slate-950 bg-indigo-500 group-hover:bg-purple-400 group-hover:scale-125 transition-all" />

              <Card className="hover:border-indigo-500/40 transition-all duration-300">
                <CardContent className="p-6 sm:p-8">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-indigo-300 font-semibold text-lg">{exp.company}</span>
                        {exp.companyUrl && (
                          <a href={exp.companyUrl} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-white">
                  ...
                  <div className="space-y-3 mb-6 bg-slate-950/60 p-4 rounded-xl border border-slate-800/80">
                    <h4 className="text-xs font-mono uppercase tracking-wider text-slate-400">Key Achievements & Impact:</h4>
